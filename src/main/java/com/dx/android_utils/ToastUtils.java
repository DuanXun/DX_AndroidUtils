...
public class ToastUtils {
     ...
     * @param gravity  设置吐司显示位置，默认显示底部
     * @param imgDrawable 显示图片
     */
    public static void init(boolean isJumpWhenMore,int gravity,int imgDrawable) {
    ...
    private static void showToast(Context context, CharSequence text, int duration) {
        ...
        sToast.setGravity(sGravity,0,0);//设置吐司显示位置，默认显示底部
        if(imgDrawable!=0){//当有传入的图片资源id的时候，吐司上显示图片
            LinearLayout toastView = (LinearLayout) sToast.getView();
            ImageView imageCodeProject = new ImageView(context.getApplicationContext());
            imageCodeProject.setImageResource(imgDrawable);
